...
static lcdPropertiesTypeDef  lcdProperties = { ILI9341_DISPLAY_WIDTH, ILI9341_DISPLAY_HEIGHT, LCD_ORIENTATION_PORTRAIT,true, true };
...
void lcdPrintf(const char *fmt, ...)
{
	...
	unsigned short cur_x = cursorXY.x; // Addition by Max Chaplin
...
//			cursorXY.x = 0;  // Modified by Max Chaplin
		}
		else if (*p == '\r')
		{
			cursorXY.x = cur_x; // Modified by Max Chaplin
		}
		else if (*p == '\t')
		{
...
void lcdSetOrientation(lcdOrientationTypeDef value)
{
	...
	switch (lcdProperties.orientation)
	{
		case LCD_ORIENTATION_PORTRAIT:
			lcdWriteData(lcdPortraitConfig);
			lcdProperties.width = ILI9341_DISPLAY_WIDTH;
			lcdProperties.height = ILI9341_DISPLAY_HEIGHT;
			break;
		...
		case LCD_ORIENTATION_LANDSCAPE:
			...
			lcdProperties.width = ILI9341_DISPLAY_HEIGHT;
			lcdProperties.height = ILI9341_DISPLAY_WIDTH;
			break;
...
/* Additional functions by Max Chaplin */

void lcdDrawPixelLine(uint16_t x, uint16_t y, uint16_t *data, uint32_t dataLength)
{
	lcdDrawPixels(x, y, data, dataLength);
}
